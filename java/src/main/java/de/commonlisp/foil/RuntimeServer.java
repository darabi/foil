...
public class RuntimeServer implements IRuntimeServer, Runnable {
        ...
        for (;;) {
            ...
            try {
                ...
                // (:proxy marshall-flags marshall-value-depth-limit interface-trefs ...)
                {
                    ...
                    resultMessage = createRetString(
                            reflector.makeProxy(this, marshallFlags, marshallDepth, message.subList(3, message.size())), marshaller,
                            IBaseMarshaller.MARSHALL_ID, 0);
                ...
                // (:cnames jarfilename packagename ...)
                {
                    StringWriter sw = new StringWriter();
                    sw.write("(:ret");
                    marshaller.marshallAsList(reflector.getClassNames((String) message.get(1), message.subList(2, message.size())), sw,
                            IBaseMarshaller.MARSHALL_ID, 1);
                    sw.write(')');
                    resultMessage = sw.toString();
                } else {
     ...
     * (non-Javadoc)
     * 
     * @see de.commonlisp.foil.IRuntimeServer#proxyCall(int, int, java.lang.reflect.Method,
