...
public class MessageReader implements IReader {
    ...
    Object readMacro(Reader strm) throws IOException, Exception {
        strm.mark(1);
        int c = strm.read();
        if (c == '{') {
            strm.reset();
    ...
    Object processMacroList(List args) throws Exception {
        if (RuntimeServer.isMessage(":box", args)) {
            ...
            return reflector.createVector(RuntimeServer.typeArg(args.get(1)), args.size() - 2, args.subList(2, args.size()));
        }
        throw new Exception("unsupported macro sequence");
    }

    static protected String readString(Reader strm) throws IOException {
        int c = strm.read();
        if (c != '"') {
    ...
    static protected Object readToken(Reader strm) throws IOException {
        ...
        for (strm.mark(1); !end; strm.mark(1)) {
            ...
            if (c == -1)
                throw new IOException("unexpected EOF");
            if (c == ')' || c == '(' || Character.isWhitespace((char) c) || c == '}') {
                strm.reset();
                end = true;
            } else
                sb.append((char) c);
    ...
    static boolean isInteger(String s) {
        ...
        for (int i = 0; ret && i < s.length(); ++i) {
            if (i == 0) {
                ret = Character.isDigit(s.charAt(i)) || s.charAt(i) == '-';
            } else
